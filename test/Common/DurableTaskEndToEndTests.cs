...
namespace Microsoft.Azure.WebJobs.Extensions.DurableTask.Tests
{
    public class DurableTaskEndToEndTests : IDisposable
    {
        ...
        [InlineData(true)]
        [InlineData(false)]
        public async Task DurableEntity_ProxyTest(bool extendedSessions)
        {
            string[] orchestratorFunctionNames =
            {
            ...
            using (var host = TestHelpers.GetJobHost(
                this.loggerProvider,
                nameof(this.DurableEntity_ProxyTest),
                extendedSessions))
            {
                await host.StartAsync();

                var proxytest = new EntityId(nameof(TestEntityClasses.ProxyTest), Guid.NewGuid().ToString());
